    ...
    <div className="w-full h-full md:px-20 px-4 sm:px-6">
      <div className="mt-12 sm:mt-16 md:mt-20 grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 md:gap-10">
        {certificateCards.map((cert, idx) => (
          <div
            key={idx}
            className="card-border rounded-xl p-4 sm:p-6 md:p-8 flex flex-col gap-4 
                      bg-white dark:bg-[#181A20] hover:bg-gray-100 dark:hover:bg-[#1f2128] 
          ...
          >
            <div className="flex items-start gap-3 sm:gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-black dark:text-white text-lg sm:text-xl font-semibold line-clamp-2">
                  {cert.title}
                </h3>
                <p className="text-gray-600 dark:text-white/60 text-sm sm:text-md mt-1">{cert.issuer}</p>
                <p className="text-gray-500 dark:text-white/40 text-xs sm:text-sm mt-1">{cert.date}</p>
              </div>
            </div>
            <p className="text-gray-700 dark:text-white/70 text-sm sm:text-base mt-2 line-clamp-3">
              {cert.description}
            </p>
